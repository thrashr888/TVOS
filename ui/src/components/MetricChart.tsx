...
export function MetricChart({ title, metrics, colors, timeRangeHours = 1 }: MetricChartProps) {
  ...
  if (chartData.length === 0) {
    return (
      <div className="glass flex h-full flex-col items-center justify-center rounded-lg p-6">
        <h3 className="mb-2 text-lg font-medium text-foreground">{title}</h3>
        <p className="text-sm text-muted-foreground">No data available for {metrics.join(', ')}</p>
      </div>
    )
  }
